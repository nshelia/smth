...
async function getMyAutoProducts(make, model, year, kilometers) {
    ...
    const response = await fetch(`https://api2.myauto.ge/ka/products?TypeID=0&ForRent=0&Mans=${make}.${model}&Cats=1&CurrencyID=1&MileageType=1&SortOrder=1&Page=1&MileageFrom=${kilometersFrom}&MileageTo=${kilometersTo}&ProdYearFrom=${year}&ProdYearTo=${year}`)
    ...
    if (response.data.meta.last_page > 1) {
        for (let page = 2; page <= response.data.meta.last_page; page++) {
            await fetch(`https://api2.myauto.ge/ka/products?TypeID=0&ForRent=0&Mans=${make}.${model}&Cats=1&CurrencyID=1&MileageType=1&SortOrder=1&Page=1&MileageFrom=${kilometersFrom}&MileageTo=${kilometersTo}&ProdYearFrom=${year}&ProdYearTo=${year}&Page=${page}`)
                .then(response => response.json())
